...
(function (w, d) {
  ...
  function queryButtons() {
    ...
    if (typeof w.DonorBox == "object" && w.DonorBox.widgetLinkClassName)
      links = arr(links).concat(
        arr(d.getElementsByClassName(w.DonorBox.widgetLinkClassName))
      );
    return links;
  }
  ...
  function addEventListener(target, ev, listener) {
    if (target.addEventListener) target.addEventListener(ev, listener, false);
    else if (target.attachEvent) target.attachEvent("on" + ev, listener);
  ...
  function openTheModal(e) {
    ...
    frame.src =
      target.href + (target.href.indexOf("?") == -1 ? "?" : "&") + "modal=true";
    ...
    if (Object.keys(UTM_PARAMS).length > 0) {
      frame.onload = function () {
        frame.contentWindow.postMessage(
          { action: "set-utm-params", msg: UTM_PARAMS },
          "*"
        );
      };
    }
  ...
  w.dw_open = function () {
    var buttons = queryButtons(),
      ...
      len = buttons.length;
    if (len == 0)
      throw "Donation widget button is not exists. If you see these on your WEB page, please, check button installation steps.";

    if (shouldOpenNewTab())
      for (; i < len; i++) buttons[i].setAttribute("target", "_blank");
    else
      for (; i < len; i++) addEventListener(buttons[i], "click", openTheModal);
  };
